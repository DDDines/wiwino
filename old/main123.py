...
best_wines_country = '''
            SELECT countries.name AS country_name, COUNT(DISTINCT wines.id) AS number_of_wines
            FROM wines
            JOIN vintages ON wines.id = vintages.wine_id
            JOIN vintage_toplists_rankings ON vintages.id = vintage_toplists_rankings.vintage_id
            JOIN toplists ON vintage_toplists_rankings.top_list_id = toplists.id
...
# 3 grapes more used in the world: 
grapes_most_used ='''
SELECT
...
vintage_avg_rating_query = '''SELECT 
...
cursor.execute(country_avg_rating_query)
print('Executed query successfully')
country_avg_ratings = cursor.fetchall()
print(cursor.fetchall())

# Plot for average rating by country
countries = [row[0] for row in country_avg_ratings]
country_ratings = [row[1] for row in country_avg_ratings]

plt.figure(figsize=(10, 8))
plt.barh(countries, country_ratings, color='skyblue')
